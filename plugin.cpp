...
PLUG_EXPORT void __cdecl CBMENUENTRY(CBTYPE cbType, PLUG_CB_MENUENTRY* info)
{
    switch(info->hEntry)
    {
	case MENU_SET_MODULES:
		DbgCmdExec("SetModules");
		break;
	case MENU_ABOUT:
        ShowAboutForm();
		break;
...
{
	_plugin_registercommand(pluginHandle, "SetModules", cbSetModules, true);
...
bool pluginStop()
{
	_plugin_unregistercommand(pluginHandle, "SetModules");
...
void pluginSetup()
{
	...
	_plugin_menuaddentry(hMenu, MENU_SET_MODULES, "&Set Modules");
